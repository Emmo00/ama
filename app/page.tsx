...
export default function Home() {
  const { user, isLoading: authLoading, error: authError, isAuthenticated } = useQuickAuth();
  const { sessions, isLoading: sessionsLoading, error: sessionsError, fetchSessions, endSession } = useSessions();
  const [userSessions, setUserSessions] = useState<any[]>([]);

  // Fetch user's sessions when user is authenticated
  useEffect(() => {
    if (user?.fid) {
      fetchSessions(undefined, user.fid.toString()).then(setUserSessions).catch(console.error);
    }
  }, [user, fetchSessions]);
      ...
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-gray-500 text-sm mb-6">
            This app requires Farcaster authentication. Please make sure you're opening this app from a Farcaster client.
          </p>
          <Button 
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {user && userSessions.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-black mb-4">Your Sessions</h2>
        ...
        <div className="mb-6">
          ...
          {sessionsError && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
              <p className="text-red-700">Failed to load sessions: {sessionsError}</p>
            </div>
          )}
          ...
          {!sessionsLoading && !sessionsError && sessions.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">Be the first to start an AMA session!</p>
              <CreateSessionModal onSessionCreated={handleSessionCreated} />
            </div>
          )}
