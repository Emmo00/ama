...
export async function GET(request: NextRequest, { params }: Params) {
  try {
    ...
    const questions = await Question.find({ sessionId: id }).sort({
      createdAt: -1,
    });

    const tips = await Tip.find({ sessionId: id }).sort({ createdAt: -1 });

    const totalTips = tips.reduce(
      (sum: number, tip: ITip) => sum + Number(tip.amount),
      0
    );
    ...
    if (body.status && ["LIVE", "ENDED"].includes(body.status)) {
      ...
      if (body.status === "ENDED" && session.status !== "ENDED") {
        ...
        const archivedStats = new ArchivedSessionStats({
          sessionId: id,
          totalTips: tips.reduce(
            (sum: number, tip: ITip) => sum + tip.amount,
            0
          ),
          totalQuestions: questions.length,
          totalParticipants: new Set([
